...
impl Store {
    fn get_current_offset() -> u64 {
        let timestamp_as_u64 = std::time::SystemTime::now()
    ...
    pub fn new() -> Store {
        ...
        let inner_size =
            ((memory.avail + memory.swap_free) as f64 * INNER_ALLOC_PERCENTAGE) as usize;
    ...
    pub fn append(&mut self, message: Vec<u8>) -> Result<usize, Box<dyn Error>> {
        if self.insert_position + message.len() > self.inner.capacity() {
            ...
            {
                let first_slice: &mut [u8] =
                    &mut self.inner[self.insert_position..self.insert_position + first_slice_size];
                first_slice.copy_from_slice(&message[0..first_slice_size]);
            }

            {
                let second_slice: &mut [u8] = &mut self.inner[0..second_slice_size - 1];
                second_slice.copy_from_slice(
                    &message[first_slice_size..first_slice_size + second_slice_size - 1],
                );
            }
